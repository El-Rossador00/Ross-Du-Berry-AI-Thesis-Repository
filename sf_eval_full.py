#!/usr/bin/env python3
# Stockfish evaluator - FULL STRENGTH
...
OUT_DIR = "sf_eval_results_full"
STOCKFISH_PATH = "/usr/local/bin/stockfish"
MULTIPV = 5
TIME_PER_MOVE = 0.5
WORKERS = max(1, mp.cpu_count() - 1)

def _analyse_full(task: Tuple[str, str, int]) -> Dict[str, int]:
    fen, played_uci, elo_bin = task
    engine = None
    try:
        engine = chess.engine.SimpleEngine.popen_uci(STOCKFISH_PATH)
        engine.configure({"Threads": 1, "Hash": 16, "UCI_LimitStrength": False})
...
def run_bin(elo_bin: int, samples: List[Dict]) -> Dict:
    ...
    out_summary_path = Path(OUT_DIR) / f"sf_eval_bin{elo_bin}_full.json"
    tasks = [(s['fen'], s['uci'], elo_bin) for s in samples]

    results: List[Dict[str, int]] = []
    with mp.Pool(processes=WORKERS) as pool:
        for r in tqdm(pool.imap_unordered(_analyse_full, tasks, chunksize=16),
                      total=len(tasks), desc=f"Analyzing bin {elo_bin} FULL"):
            if r: results.append(r)

    n = len(results)
    summary = {
        "bin": elo_bin, "mode": "full", "positions_analyzed": n,
    ...
    print(f"[bin {elo_bin}] FULL results -> {out_summary_path}")
    return summary
